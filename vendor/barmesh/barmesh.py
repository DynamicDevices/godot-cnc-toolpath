...
class BarMesh:
    ...
    def AddNode(self, node):
        ...
        node.i = len(self.nodes)
        self.nodes.append(node)
        return self.nodes[-1]

    def NewNode(self, p):
        return self.AddNode(Node(p, -1))

    def NewIntermediateNode(self, lam, node0, node1):
        return self.AddNode(node0.newnodetowardsothernode(node1, lam, -1))
    ...
    def InsertNodeIntoBarF(self, bar, newnode, bcolinear):
        ...
        if bcolinear:  # copy over so we can identify colinearity from creation and not attempt to split along it
            assert (bar.barvecN - barback.barvecN).Len() < 1e-5, (bar.nodeback.p, newnode.p, bar.nodefore.p, bar.barvecN, barback.barvecN)
            assert (bar.barvecN + barfore.barvecN).Len() < 1e-5, (bar.nodeback.p, newnode.p, bar.nodefore.p, bar.barvecN, barback.barvecN) 
    ...
    #    |       |    
    #   b1       |   
    def MakeBarBetweenNodesF(self, node1, bar1, node2, bar2, strictlyplanarbarmesh=True):
